...
tags: [Customer Segmentation, Machine Learning, Clustering]
---

This project uses k-means clustering to segment up the customer base in order to increase business understanding, and to enhance the relevancy of targeted messaging & customer communications.
...
The first step is to compile the necessary data from several tables in the database, namely the *transactions* table and the *product_areas* table.  These are joined together for the relevant information using Pandas, and then the data is aggregated across product areas, from the most recent six month to a customer level.  The final data for clustering is the percentage of sales allocated to each product area for each customer.
...
Again, k-means is a distance based algorithm. In other words, it is reliant on an understanding of how similar or different data points are across different dimensions in n-dimensional space, so the application of Feature Scaling is extremely important.
...
In the code below, we will test multiple values for k and plot how this WCSS metric changes.  As we increase the value for k (i.e., as we increase the number or centroids or clusters), the WCSS value will always decrease.  However, these decreases will get smaller and smaller each time we add another centroid and we are looking for a point where this decrease is quite prominent *before* this point of diminishing returns.
...
Once we have our data separated into distinct clusters, our client needs to understand *what is is* that is driving the separation.  This means the business can understand the customers within each and the behaviors that make them unique.

<br>
##### Cluster Sizes

In the below code, we assess the number of customers that fall into each cluster.
...
Even though this is a simple solution based upon high level product areas, it will help leaders and category managers gain a clearer understanding of the customer base.
